unique template common/logging/server;

include 'site/backup';

include 'common/logging/drop-old';

variable SYSLOG_GROK_PATTERNS ?= {
    # keep this list in sync with logstash-patterns rpm
    patterns = list('ssh', 'modulecmd', 'nfs', 'ceph', 'opennebula', 'jube', 'keyvalue', 'quattor', 'snoopy');
...
"filter/conditionals" = append(dict(
    ...
    "plugins", list(
        ...
        dict("mutate", dict(
            ...
            "update", dict(escape("[@metadata][target_index]"), "longterm-%{+YYYY.Q}"),
            )),
        ),
    )
);
