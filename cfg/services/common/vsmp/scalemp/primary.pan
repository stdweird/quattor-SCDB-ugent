...
variable VSMP_SCALEMP_IP_POLICY = 'alias';
...
"/" = {
    foreach(idx;sl;VSMP_SCALEMP_CONFIG["slaves"]) {
        ...
        foreach(name;nic;value(format("%s%s:%s",SCALEMP_PROFILE_PREFIX,sl,path))) {
            ...
            idx=1;
            # use while to traverse the current tree
            ...
            if (is_defined(ip)) {
                if(VSMP_SCALEMP_IP_POLICY == 'exact') {
                    ...
                    if(VSMP_SCALEMP_IP_POLICY == 'alias') {
                        SELF['system']['network']['interfaces'][origname]['aliases'][name]= dict(
                            'ip', ip['ip'],
                            'netmask', ip['netmask'],
