...
variable VSMP_SCALEMP_ORIGCORES=cores_in_hardware(value("/hardware"));
include 'common/vsmp/scalemp/schema';
## this will conflict with cloning
variable SCALEMP_MASTER_MAP ?= dict(
...
# disable cpuspeed (again?)
"/software/components/chkconfig/service/cpuspeed" = dict("off", "", "startstop", true);
...
"/system/licenses" = append(dict(
             "vendor", "ScaleMP",
             "name", "vSMP Foundation",
             "enddate", SCALEMP_SUPPORT_ENDDATE,
             ));
