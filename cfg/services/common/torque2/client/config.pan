...
"directPaths" = {
  ...
  if ( length(wn_mnts) > 0 ) {
    ...
    while (ok) {
      ...
      directpaths[length(directpaths)] =
    	              #dict("locations",'*.'+DOMAIN+':'+mnt_point,
                      dict("locations",'*'+':'+mnt_point,
    	                    "path",mnt_point,
    		           );
  ...
  # add /home to directpaths for backward compatibility
    ...
    directpaths[length(directpaths)] = dict("locations",'*.'+DOMAIN+':/home',
                                             "path","/home"
                                            );
  };
...
"args" = value("/software/components/sysconfig/files/pbs_mom/PBS_ARGS");
