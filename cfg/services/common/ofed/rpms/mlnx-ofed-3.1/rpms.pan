unique template common/ofed/rpms/mlnx-ofed-3.1/rpms;

final variable MLX4_LOG_NUM_MGM_ENTRY_SIZE = -1;

variable OS_REPOSITORY_LIST = append("mlnx_ofed_31");

variable OFED_VERSION ?= "3.1";
variable OFED_RELEASE ?= "1.0.3.1.g9032737";
variable OFED_DISTRO ?= undef;

variable OFED_KERNEL_MFT_VERSION ?= "4.1.0";
...
"/software/packages" = {
    ...
    pkg_repl("mlnx-ofa_kernel-modules*", format("%s-%s_OFED.%s.%s", OFED_VERSION, OFED_KERNEL_VERSION_FIXED, OFED_VERSION, OFED_RELEASE), PKG_ARCH_DEFAULT);
