...
include { format("%sbase", OS_NS_CONFIG_BASE) };
...
include { format('%saii', OS_NS_QUATTOR) };


# Add local users if some configured
variable USER_CONFIG_INCLUDE = {
    if ( exists(USER_CONFIG_SITE) && is_defined(USER_CONFIG_SITE) ) {
        'users/config';
    } else {
        null;
    };
...
include CORE_CONFIG_SITE;
# The template is called by machine-types templates.
variable OS_POSTCONFIG_TYPE ?= 'core';
variable OS_POSTCONFIG ?= format('config/%s/postconfig', OS_POSTCONFIG_TYPE);
